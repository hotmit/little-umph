...
namespace LittleUmph
{
    ...
    public class IOFunc
    {
        ...
        /// <param name="newFileName">New name of the file (with or without extension).</param>
        /// <returns></returns>
        public static bool Rename(FileInfo file, string newFileName)
        {
            try
            {
                ...
                string orgExt = Path.GetExtension(newFileName);
                if (orgExt == null || orgExt.Length == 0)
                {
        ...
        public static bool Rename(DirectoryInfo dir, string newFolderName)
        {
            try
            {
                ...
                if (string.Compare(oldName, newName, ignoreCase: true) == 0)
                {
                    ...
                    Directory.Move(dir.FullName, tempName);
        ...
        private static string PrepCmd(string cmd)
        {
            ...
            if (cmd[1] != ':')
            {
                ...
                string localPath = ResolvePath(currectDirectory, cmd);

                // only resolve path if the local exe exist
        ...
        /// <returns></returns>
        public static bool MoveToDirectory(DirectoryInfo dir, string newFolderName, IEnumerable<FileInfo> fileList)
        {
            try
            {
                string newPath = Path.Combine(dir.FullName, newFolderName);
                ...
                {
                    MoveFile(f, newPath, ConflictRules.AutoRename);
                }
        ...
        public static bool FolderIsEmpty(DirectoryInfo dir)
        {
            if (!Directory.Exists(dir.FullName))
            {
                // when it doesn't exist it is clean
                return true;
            }
        ...
        #region [ Crush It ]
        /// <summary>
        /// Move the contents of all the sub folder into the root directory.
        /// </summary>
        /// <param name="root">The root.</param>
        public static void CrushFolder(DirectoryInfo root)
        {
            CrushFolder(root, 10);
        }

        private static void CrushFolder(DirectoryInfo root, int pass)
        {
            if (pass < 0)
            {
                return;
            ...
            DirectoryInfo[] allDirs = root.GetDirectories();
            foreach (DirectoryInfo dir in allDirs)
            {
                try
                {
                    IOFunc.MoveDirectoryContents(dir, root.FullName, IOFunc.ConflictRules.OverwriteIfIdentical);
                    if (IOFunc.FolderIsEmpty(dir))
                    {
                        dir.Delete();
                    }
                }
        ...
        #region [ Merge IT ]
        /// <summary>
        /// Append the name of the subfolder to its contents 
        /// and move it to the root directory.
        /// </summary>
        /// <param name="root">The root.</param>
        public static void MergeFolder(DirectoryInfo root)
        {
            MergeFolder(root, root, 10);
        }

        private static void MergeFolder(DirectoryInfo root, DirectoryInfo current, int pass)
        {
            if (pass < 0)
            {
                return;
            ...
            DirectoryInfo[] allDirs = current.GetDirectories();
            foreach (DirectoryInfo dir in allDirs)
            {
                if (dir.GetDirectories().Length > 0)
                {
                    MergeFolder(root, dir, --pass);
                }
                ...
                foreach (FileInfo file in dir.GetFiles())
                {
                    try
                    {
                        string newPath = file.FullName.Replace(root.FullName, "");
                        newPath = root.FullName + "\\" + newPath.Replace("\\", " ").Trim();

                        if (File.Exists(newPath))
                        {
                            if (IOFunc.IsSameFile(file.FullName, newPath))
                            {
                                File.Delete(newPath);
                            }
                            else
        ...
        /// <summary>
        /// Recursively gets the directories in the sub folders.
        /// </summary>
        /// <param name="root">The root.</param>
        /// <param name="filter">The filter.</param>
        /// <param name="depth">The depth.</param>
        /// <returns></returns>
        public static List<DirectoryInfo> GetDirectories(DirectoryInfo root, string filter = "*.*", int depth = -1)
        {
            ...
            DirectoryInfo[] directories = root.GetDirectories(filter);
            list.AddRange(directories);

            foreach (var d in directories)
            {
                List<DirectoryInfo> dirs = GetDirectories(d, filter, --depth);
                if (dirs.Count > 0)
                {
                    list.AddRange(dirs);
                }
        ...
        /// <summary>
        /// Gets the files in specified dir and its sub directories.
        /// </summary>
        /// <param name="dir">The dir.</param>
        /// <returns></returns>
        ...
        /// <returns></returns>
        public static DirectoryInfo CreateDirectory(DirectoryInfo root, string dirName)
        {
            string dir = Path.Combine(root.FullName, dirName);
            CreateDirectory(dir);
        ...
        public static void BatchRename(FileInfo file, string newName, params string[] extensions)
        {                        
            string name = Path.GetFileNameWithoutExtension(file.Name);
            List<string> extList = new List<string>(extensions);

            if (!Arr.Contains(extList, file.Extension))
            {
