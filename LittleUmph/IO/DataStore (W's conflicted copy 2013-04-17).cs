    ...
    public class DataStore
    {
        ...
        public void EncryptSet(string groupName, string name, object data, string passPhrase)
        {
            try
            {
                ...
                if (data is Image || data is Bitmap)
                {
                    data = Img.ToBytes((Image)data);
                }

                string strData = "";
                bool needChildNode = false;
                Type dataType = data.GetType();

                if (data is DateTime)
                {
                    DateTime dt = (DateTime)data;
                    strData = dt.ToString("MM/dd/yyyy hh:mm:ss.fff tt");
                }
                else if (data is TimeSpan)
                {
                    TimeSpan ts = (TimeSpan)data;
                    strData = ts.Ticks.ToString();
                }
                else if (data is byte[])
                {
                    byte[] arr = (byte[])data;
                    string hex = ByteArr.ToHex(arr);
                    strData = hex;
                }
                else if (data is Color)
                {
                    Color c = (Color)data;
                    strData = ((int)((c.A << 24) | (c.R << 16) | (c.G << 8) | (c.B << 0))).ToString();
                }
                else if (ShouldSerialize(data.GetType()))
                {
                    XmlNode serializedNode = Serializer.SerializeXMLNode(data);
                    strData = "";
                    ...
                    if (Str.IsEmpty(passPhrase))
                    {
                        ...
                        XmlDocumentFragment frag = _xmlDoc.CreateDocumentFragment();
                        frag.InnerXml = serializedNode.OuterXml;
                        node.AppendChild(frag);
                    }
                    else
                    {
                        strData = serializedNode.OuterXml;
                    }
                }
                else
                {
                ...
                if (!needChildNode && node.HasChildNodes)
                {
                    // Remove all children
                    node.RemoveAll();
                }
        ...
        public T EncryptGet<T>(string groupName, string name, T defaultValue, string passPhrase)
        {
            try
            {
                ...
                #region [ Special DataType Convertion ]
                if (value.Length == 0 && node.Attributes.Count > 0)
                {
                    return default(T);
                }
                ...
                {
                    Color c = Color.FromArgb(Convert.ToInt32(value));
                    return (T)(object)(c);
                }
                else if (typeof(Enum).IsAssignableFrom(typeof(T)))
                {
                    object enu = Enum.Parse(typeof(T), value, true);
                    return (T)(enu);
                }
                else if (ShouldSerialize(typeof(T)))
                {
                    if (node.HasChildNodes && node.ChildNodes[0].NodeType != XmlNodeType.Text)
                    {
                        return Serializer.DeserializeXMLNode<T>(node.ChildNodes[0]);
                    }
                    else
                    {
                        return Serializer.DeserializeXML<T>(value);
                    }
                }
                #endregion
        ...
        #region [ SaveToFile ]
        /// <summary>
        /// Saves all the current settings to xml dir.
        /// </summary>
        ...
        public void Load(object obj)
        {
            ...
            {
                var c = (FolderBrowserDialog)obj;
                c.SelectedPath = Get("FolderBrowserDialog", "");
            }
            else if (obj is OpenFileDialog)
            {
                var c = (OpenFileDialog)obj;
                c.InitialDirectory = Get("OpenFileDialog", "");
            }
        }
